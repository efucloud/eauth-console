...
import { AccessTokenResponse, RegisterByOIDC, MfaCode, LoginByOIDC, LoginByLDAP, Logout, AuthedUserInfo, OidcCodeResponse, OidcCodeRequest, LoginByUsername } from './common.d';
...
export async function systemIntrospect(
  ...
  const { token_type_hint, token, ...rest } = params;
...
//退出登录
//退出登录,会同时退出所有的组织，清理对应的cookie，但是token任有效
//请求方法: POST
//请求地址: /api/logout
export async function systemLogout(  data: Logout,   options?: { [key: string]: any }) {
...
export async function systemRevoke(
  ...
  options?: { [key: string]: any }) {
  const { client_id, client_secret, token, token_type_hint, ...rest } = params;
